...
def main():
    ...
    PRI_PATH_FILE = 'data/anc/Primary_path.mat'
    SEC_PATH_FILE = 'data/anc/Secondary_path.mat'
    FILTERS_FILE = 'data/anc/Pretrained_Control_filters.mat'

    print(f" Bắt đầu chương trình phân loại dataset ")
    ...
    for sub, target in zip(subfolders, target_names):
        ...
        if not os.path.exists(input_dir):
            print(f"Bỏ qua: Không tìm thấy thư mục {input_dir}")
            continue
        ...
        if len(files) == 0:
            print(f"Thư mục {sub} trống, bỏ qua.")
            continue
